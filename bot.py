...
if not TOKEN:
    raise ValueError("🚨 DISCORD_TOKEN이 설정되지 않았습니다. GitHub Secrets를 확인하세요.")

if not CHANNEL_ID:
    raise ValueError("🚨 CHANNEL_ID가 설정되지 않았습니다. GitHub Secrets를 확인하세요.")

CHANNEL_ID = int(CHANNEL_ID)  # 숫자로 변환

# 디스코드 클라이언트 설정
intents = discord.Intents.default()  # 메시지 관련 최소한의 권한만 설정
...
# GitHub Actions에서 실행하기 위해 asyncio.run() 사용
